...
<template>
    <AppLayout :breadcrumbs="breadcrumbs">
        <Head title="Pointage — validation manager" />

        <div class="flex flex-col gap-6 p-6">
            <div>
                <h1 class="text-2xl font-semibold tracking-tight">Déclarations à valider (manager)</h1>
                <p class="mt-1 text-sm text-muted-foreground">
                    Réservé aux N+1 des collaborateurs (les administrateurs et RH voient l’ensemble des dossiers en attente manager).
                </p>
            </div>
                    ...
                    <div class="flex flex-wrap items-start justify-between gap-4">
                        ...
                        <div class="w-full min-w-[200px] max-w-md space-y-2 sm:w-auto">
                            ...
                            <div class="flex flex-wrap gap-2">
                                <Button size="sm" @click="decide(d.id, 'approve')">Transmettre RH</Button>
                                <Button size="sm" variant="destructive" @click="decide(d.id, 'reject')">Rejeter</Button>
                            </div>
                        </div>
                    </div>
                </div>
                <p v-if="!declarations.length" class="text-center text-sm text-muted-foreground">Aucune déclaration en attente.</p>
            </div>
        </div>
    </AppLayout>
</template>
