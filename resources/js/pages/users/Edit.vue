...
const form = useForm({
    ...
    roles: (props.user.roles?.map((role) => Number(role.id)) ?? []) as number[],
...
watch(
    ...
    (newProfilId) => {
        ...
        if (suggestedId && !selectedRoleIds.value.includes(suggestedId)) {
            form.roles = [...selectedRoleIds.value, suggestedId];
        }
    },
);
...
const submit = () => {
    form
        ...
        .put(`/users/${props.user.id}`, { preserveScroll: true });
};
</script>
